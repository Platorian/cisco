...
```php
for i in {1..24}; do (ping -c 1 172.16.0.${i} | grep "bytes from" &); done
```
...
- Will show you any IP addresses of hosts that the target has interacted with recently
Windows
```php
`ipconfig /all`
```
