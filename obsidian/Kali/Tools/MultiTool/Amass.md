...
Simple whois.
```
amass intel -whosi <Taget.com> -dir <Output Dir>
```
...
Enumeration.
```
amass enum -d <Target.com> -dir <Output Dir>
```

Ask for sources and ip adresses.
```
amass enum -d <Target.com> -src -ip -dir <Output Dir>
```

Sub directories.
```
amass -d <Target.com> -src -ip -brute -dir <Output Dir>
```

We can use Viz to output our information into different formats.
Can output into a  Maltego file.

List reports
```
amass db -dir <The Output File> -list
```

Viz converts it to html, but can do many other formats.
```
amass viz -dir <Output File> -d3
```
